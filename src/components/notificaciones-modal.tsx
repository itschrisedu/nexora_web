...
export default function NotificacionesModal({
  ...
  const enviarEmailDirecto = async (cobro: any) => {
    if (!cobro.clienteEmail) {
      showToast('Este cliente no tiene un correo registrado', 'error');
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      ...
      >
        {/* ── HEADER ── */}
        <div className="p-3.5 sm:p-5 border-b border-[var(--border)] flex items-center justify-between bg-[var(--muted)]/40 shrink-0 gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="text-sm sm:text-lg font-black tracking-tight truncate">Centro de Notificaciones & Cobranza</h2>
                {data.metricas.totalAlertas > 0 && (
                  <span className="px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-black bg-rose-500/15 text-rose-500 border border-rose-500/30 animate-pulse shrink-0">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            <button
              ...
              disabled={loading}
              className="p-1.5 sm:p-2 rounded-xl border border-[var(--border)] bg-[var(--card)] hover:bg-[var(--muted)] text-[var(--foreground)] transition-colors cursor-pointer text-xs font-bold flex items-center gap-1.5 shadow-2xs"
            ...
            <button
              onClick={onClose}
              className="p-1.5 sm:p-2 rounded-xl text-[var(--muted-foreground)] hover:text-[var(--foreground)] hover:bg-[var(--muted)] transition-colors cursor-pointer"
            >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-2.5 p-2.5 sm:p-4 bg-[var(--muted)]/15 border-b border-[var(--border)] shrink-0">
          ...
          <div className="p-2 sm:p-2.5 rounded-xl bg-[var(--card)] border border-blue-500/20 flex items-center gap-2 sm:gap-3 shadow-2xs min-w-0 overflow-hidden">
            <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-blue-500/10 text-blue-500 flex items-center justify-center shrink-0">
              <Package size={15} />
            </div>
            <div className="min-w-0 flex-1">
              <div className="text-[9px] sm:text-[10px] uppercase font-black text-blue-500/80 truncate">Stock Crítico</div>
              <div className="text-xs sm:text-sm font-black text-blue-500 truncate">{data.metricas.totalStockCritico} modelos</div>
              <div className="text-[9px] sm:text-[10px] text-[var(--muted-foreground)] truncate">Pares agotándose</div>
            </div>
          </div>
        ...
        {/* ── TABS DE NAVEGACIÓN ── */}
        <div className="flex border-b border-[var(--border)] px-3 sm:px-4 bg-[var(--card)] gap-1 sm:gap-2 overflow-x-auto shrink-0 no-scrollbar">
          <button
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-3 sm:p-6 space-y-4">
          ...
          {activeTab === 'cobros' && (
            <div className="space-y-4">
              {/* Filtros de Cobro */}
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5 bg-[var(--muted)]/30 p-2.5 rounded-xl border border-[var(--border)]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 w-full">
                  {cobrosFiltrados.map((cobro) => {
                    ...
                    return (
                      <div
                        key={cobro.id}
                        className={`p-3.5 sm:p-4 rounded-xl border transition-all flex flex-col justify-between shadow-2xs hover:shadow-md min-w-0 overflow-hidden ${
                          esVencido
                      ...
                      >
                        <div className="space-y-2 min-w-0">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="text-[11px] text-[var(--muted-foreground)] flex items-center gap-2 mt-0.5 flex-wrap">
                                ...
                                <span className="flex items-center gap-0.5 truncate">
                                  <Building2 size={11} className="shrink-0" /> {cobro.sucursalNombre}
                                </span>
                              </div>
                            </div>

                            <span
                              className={`px-2 py-0.5 rounded-full text-[10px] font-black border uppercase tracking-wider shrink-0 whitespace-nowrap ${
                                esVencido
                          ...
                          <div className="p-2.5 rounded-lg bg-[var(--card)] border border-[var(--border)] flex items-center justify-between gap-2">
                            <div className="min-w-0">
                              <div className="text-[10px] font-bold text-[var(--muted-foreground)] uppercase truncate">Saldo Pendiente</div>
                              <div className="text-sm sm:text-base font-black text-rose-500">${Number(cobro.saldoPendiente || 0).toFixed(2)}</div>
                            </div>
                            <div className="text-right min-w-0">
                              <div className="text-[10px] font-bold text-[var(--muted-foreground)] uppercase truncate">Total Nota</div>
                              <div className="text-xs font-bold text-[var(--foreground)]">${Number(cobro.montoTotal || 0).toFixed(2)}</div>
                            </div>
                          </div>

                          <div className="text-[11px] text-[var(--muted-foreground)] flex items-center justify-between flex-wrap gap-1">
                            <span className="flex items-center gap-1 truncate">
                              <Phone size={11} className="shrink-0" /> {cobro.clienteTelefono || 'Sin teléfono'}
                            </span>
                        ...
                        <div className="pt-3 mt-3 border-t border-[var(--border)] flex items-center gap-2">
                          ...
                          >
                            ...
                            <span className="truncate">Cobrar WhatsApp</span>
                          </button>

                          {cobro.clienteEmail && (
                            <button
                              onClick={() => enviarEmailDirecto(cobro)}
                              className="p-2 rounded-lg text-xs font-bold bg-[var(--card)] border border-[var(--border)] hover:bg-[var(--muted)] text-[var(--foreground)] transition-colors cursor-pointer shrink-0"
                              title="Enviar recordatorio formal por Correo"
                            >
                              <Mail size={14} />
                            </button>
                          )}
              ...
              {historialLogs.length === 0 ? (
                ...
                <div className="space-y-2">
                  {historialLogs.map((log) => (
                    <div
                      key={log.id}
                      className="p-3 rounded-xl bg-[var(--card)] border border-[var(--border)] flex items-center justify-between gap-3 text-xs shadow-2xs min-w-0 overflow-hidden flex-wrap sm:flex-nowrap"
                    >
        ...
        {/* ── FOOTER ── */}
        <div className="p-3 sm:p-4 border-t border-[var(--border)] bg-[var(--muted)]/30 flex items-center justify-between text-xs text-[var(--muted-foreground)] shrink-0 gap-2">
          <div className="flex items-center gap-1.5 min-w-0 truncate">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shrink-0" />
            <span className="truncate">Notificaciones automáticas sincronizadas con el servidor</span>
          </div>
          <button
            onClick={onClose}
            className="px-4 py-1.5 rounded-xl font-bold bg-[var(--primary)] text-white hover:opacity-90 transition-opacity cursor-pointer shrink-0"
          >
            Cerrar
          </button>
        </div>
          ...
          <div className="w-full max-w-xl rounded-2xl bg-[var(--card)] text-[var(--foreground)] border border-[var(--border)] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-3.5 sm:p-4 border-b border-[var(--border)] flex items-center justify-between bg-emerald-600/10 shrink-0 gap-2">
              ...
              <button
                ...
                className="p-1.5 rounded-lg text-[var(--muted-foreground)] hover:text-[var(--foreground)] shrink-0"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-3.5 sm:p-5 overflow-y-auto space-y-4 flex-1">
              ...
              <div>
                ...
                <div className="p-3.5 rounded-2xl bg-[#0b141a]/95 text-emerald-50 border border-emerald-900/40 text-xs font-sans whitespace-pre-wrap break-words leading-relaxed shadow-inner max-h-48 overflow-y-auto">
                  {obtenerTextoMensaje()}
                </div>
              </div>
            </div>
            ...
            {/* Footer de Acciones */}
            <div className="p-3.5 sm:p-4 border-t border-[var(--border)] bg-[var(--muted)]/30 flex items-center justify-between gap-3 shrink-0">
              <button
                onClick={() => setRecordatorioModalOpen(false)}
              ...
              <button
                ...
                className="px-4 sm:px-5 py-2.5 rounded-xl text-xs font-black text-white bg-emerald-600 hover:bg-emerald-500 transition-all flex items-center gap-2 shadow-md cursor-pointer disabled:opacity-50"
              >
                <Send size={15} />
